...
#        print log['peptides'][pep].keys()
        for f in log['peptides'][pep]['System Suitability']['Failed']:
                iRT_dict[f]+=1
...
def parse_suitability(val):
    ...
    ret['Failed']=[]
    if "Failed" in val:
        ret['Failed']=val.split("Failed")[-1].split()[1:]
    if "Passed:" in val:
        ps = val.split("Passed:")[-1]
        if ";" in ps:
            ps = ps.split(";")[0]
        ret['Passed'] = ps.split()
    if len(ret['Passed'])+len(ret['Failed'])>0:
        ret['Fail_pc'] = len(ret['Failed'])*100.0/(len(ret['Failed'])+len(ret['Passed']))
    else:
        ret['Fail_pc'] = None
    return ret
        ...
        if line.startswith("iRT-pep_"):
            ...
            for param, val in zip(params, tokens[1:]):
                if 'N/A' in val:
                    ...
                    print "Found suitability",val
                    val = parse_suitability(val)
                else:
                    val = float(val)
                peptide[param] = val
#    print log['peptides']
...
def make_pep_id_params(pep_ids, param):
    ...
    temp = ['peptides',None]
    if type(param)==list:
        print "Found a list of params",param
        temp.extend(param)
    else:
        print "Found a single param",param
        temp.append(param)
    for pep_id in sorted(pep_ids):
        temp[1]=pep_id
        pep_id_params.append([pep_id, temp[:]])
    print pep_id_params
